...
import com.moonring.jetpackmvvm.base.activity.BaseVmActivity
import com.moonring.jetpackmvvm.base.fragment.BaseVmFragment
 ...
 * @param onError 失败回调
 * @param isSuccessDismissLoading 是否需要dismissloading
 *
 */
fun <T> BaseVmActivity<*>.parseState(
    ...
    onLoading: (() -> Unit)? = null,
    isSuccessDismissLoading:Boolean = true
) {
    when (resultState) {
 ...
 *
 */
fun <T> BaseVmFragment<*>.parseStateNoLoading(
    resultState: ResultState<T>,
    onSuccess: (T) -> Unit,
    onError: ((AppException) -> Unit)? = null,
) {
 ...
 * 延时隐藏loading
 */
fun <T> BaseViewModel.dismissLoadingDelay(timeMillis: Long = 1500,block: () -> T){
    viewModelScope.launch {
        delay(timeMillis)//网络请求成功 关闭弹窗
        loadingChange.dismissDialog.postValue(false)
        block()
    }
}
 ...
 * 延时隐藏loading
 */
fun  BaseViewModel.dismissLoading(){
    viewModelScope.launch {
        loadingChange.dismissDialog.postValue(false)
 ...
 * 延时执行
 */
fun <T> BaseViewModel.delay(timeMillis: Long = 1500,block: () -> T){
    viewModelScope.launch {
        delay(timeMillis)//网络请求成功 关闭弹窗
        block()
    }
